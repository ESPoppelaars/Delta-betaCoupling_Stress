BF_t<-function(meantheory, sdtheory, dftheory, meanobtained, semobtained, dfobtained, mytitle = NULL, tail = 2)
...

{
  ...
  if (theta0 <= 0) {
    mylegend = "L"   # <---- define legend on right ("R") or left
  } else {
  ...
  mypie = F  # <---- include pie chart, T or F
  # ==========================================
  if (mypie == T) {
  ...
  plot(theta, dist_theta_alt, type = "l",
       ...
       ylab = "Density", xlab = "Theta", col = "blue", lwd = 7, lty = 5)
  lines(theta, height_alt, type = "l", col = "red", lwd = 7, lty = 5)
  text(legend_coor,maxy-(maxy/10*1), "Prior", col = "blue", adj = legend_adj, font = 2)
  text(legend_coor,maxy-(maxy/10*2), "Posterior", col = "red", adj = legend_adj, font = 2)
